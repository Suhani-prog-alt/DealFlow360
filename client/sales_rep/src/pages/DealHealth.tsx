    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-400">Identify stalled deals, high-risk quotes, and discount anomalies</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-5 bg-[#1a1a1a] border border-[#333] rounded-lg">
          <div className="text-sm font-medium text-zinc-400 mb-2">Healthy</div>
          <div className="text-3xl font-light text-green-400">{healthyQuotes.length}</div>
        </div>
        <div className="card p-5 bg-[#1a1a1a] border border-[#333] rounded-lg">
          <div className="text-sm font-medium text-zinc-400 mb-2">Warning</div>
        ...
        <div className="card p-5 bg-[#1a1a1a] border border-[#333] rounded-lg">
          <div className="text-sm font-medium text-zinc-400 mb-2">Stalled</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="card bg-[#1a1a1a] rounded-lg border border-[#333] p-5">
          ...
          <div className="space-y-4">
            {highRiskQuotes.map(q => (
              <div key={q.id} className="p-4 bg-[#111] border border-[#333] rounded">
                <div className="flex justify-between items-start mb-2">
                  <div className="font-medium text-white">{q.id.split('-')[0]} <span className="text-zinc-400 font-normal text-sm ml-2">{q.customer}</span></div>
            ...
            {highRiskQuotes.length === 0 && <p className="text-zinc-400 text-sm">No high risk deals found.</p>}
          </div>
        </div>
            ...
            <div className="space-y-4">
              {stalledQuotes.map(q => (
                <div key={q.id} className="p-4 bg-[#111] border border-[#333] rounded">
                  ...
                  <div className="text-sm text-zinc-400 mb-3">Customer: {q.customer}</div>
                  ...
                  <div className="text-xs text-zinc-400 mb-4">Last activity: {new Date(q.createdAt).toLocaleDateString()}</div>
                  <div className="flex space-x-2">
                    <button onClick={() => handleAction('View Quote', q.id)} className="text-xs bg-[#222] hover:bg-[#333] text-white px-3 py-1.5 rounded transition">View Quote</button>
              ...
              {stalledQuotes.length === 0 && <p className="text-zinc-400 text-sm">No stalled deals found.</p>}
            </div>
          </div>
              ...
              <div className="p-4 bg-[#111] border border-[#333] rounded">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-zinc-400">Rep average discount:</span>
                    <span className="text-white">8%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-zinc-400">Current quote amount:</span>
