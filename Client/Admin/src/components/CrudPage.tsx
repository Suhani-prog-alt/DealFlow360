...
export default function CrudPage({ title, description, model, columns }: CrudPageProps) {
  ...
  const renderInput = (col: Column) => {
    ...
    const isNum = typeof val === 'number' || col.key.toLowerCase().includes('margin') || col.key.toLowerCase().includes('price') || col.key.toLowerCase().includes('discount') || col.key.toLowerCase().includes('weight');
  ...
  return (
    <div className="max-w-5xl space-y-6 font-sans relative">
      {toastMsg && (
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 bg-[#7d9b6b] text-[#0f1110] px-4 py-2 rounded shadow-lg text-sm font-medium z-40 transition-opacity">
          {toastMsg}
        </div>
      )}
      <div className="flex justify-between items-center">
        ...
        <button onClick={handleAdd} className="bg-[#7d9b6b] hover:bg-[#8cae78] text-[#0f1110] font-medium px-4 py-2 rounded-md text-sm transition-colors border border-transparent flex items-center">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#212623] bg-[#1a1e1b]">
                {columns.map(col => (
                  <th key={col.key} className="p-4 text-xs font-medium text-gray-400 uppercase tracking-wider">
                    {col.label}
                  </th>
                ))}
